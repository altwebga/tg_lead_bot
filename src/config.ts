...
function encodeCredentialComponent(value: string): string {
  try {
...
function encodeProxyCredentials(proxyUrl: string): string {
...
function normalizeProxyUrl(proxyUrl?: string): string | undefined {
  ...
  try {
    ...
    if (error instanceof TypeError || error instanceof URIError) {
      throw new Error(
        `PROXY_URL некорректен: ${error.message}. Используйте формат socks5://user:password@host:port`
      );
    }
